...
app.use(async (ctx, next) => {
  await next();
  // 参数出现在HTML内容或属性浏览器会拦截 默认是1浏览器自动拦截xss
...
router.post("/login", async (ctx) => {
  ...
  // 正确写法
  const sql = `
    SELECT * FROM test.user WHERE username = ? AND password = ?
    `;
  ...
  //   `;
  // res = await query(sql, [username]); //查找用户
  // if (res.length !== 0 && res[0].salt === null) { 用户没有加言
  //     console.log('no salt ..')
  //     if (password === res[0].password) {//对密码进行加言加密
  //         sql = `
  //             update test.user
  //             set salt = ?,
  //             password = ?
